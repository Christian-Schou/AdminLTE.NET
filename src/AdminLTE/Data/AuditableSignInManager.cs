...
public class AuditableSignInManager<TUser> : SignInManager<TUser> where TUser : class
{
    ...
    public AuditableSignInManager(UserManager<TUser> userManager, IHttpContextAccessor contextAccessor,
        IUserClaimsPrincipalFactory<TUser> claimsFactory, IOptions<IdentityOptions> optionsAccessor,
        ILogger<SignInManager<TUser>> logger, ApplicationDbContext dbContext)
    ...
    {
        await _contextAccessor.HttpContext.SignOutAsync(_contextAccessor.HttpContext.Features
            .Get<IAuthenticateResultFeature>().AuthenticateResult?.Ticket?.AuthenticationScheme);

        var user =
            await _userManager.FindByIdAsync(_userManager.GetUserId(_contextAccessor.HttpContext.User)) as IdentityUser;
